...
```{r}
sc_controls <- sc %>% filter(Group == 0 & ID %in% c(201, 202, 203))

# data frames with data for the stan model
df_choice <- sc_controls %>% subset(select=c(ID, SecondRating, Trial_Round1)) %>% 
  pivot_wider(names_from = ID, values_from = SecondRating)

df_s1 <- sc_controls %>% subset(select=c(ID, FirstRating, Trial_Round1)) %>% 
  pivot_wider(names_from = ID, values_from = FirstRating)

df_s2 <- sc_controls %>% subset(select=c(ID, OtherRating, Trial_Round1)) %>% 
  pivot_wider(names_from = ID, values_from = OtherRating)
```
...
```{r}
# data for the stan model
n_trials <- max(sc_controls$Trial_Round1)
n_participants <- length(unique(sc_controls$ID))
